...
import { skinUrl, convertToFormdata, ConvertObjectToArrayErrors, api } from "../constants/global";
...
function RegistrationModalAffiliate(props) {

    var url_string = window.location;
    var url = new URL(url_string);

    const codicePromo = url.searchParams.get('promocode');
    const affiliate_id = url.searchParams.get('affiliate_id');

    const [codicePromotore, setCodicePromotore] = useState('');
    ...
    const [inputs, setInputs] = useState({"conditions":false,"years":false, "model":"classic", "affiliate_id":affiliate_id, "promocode": codicePromo});
    ...
    const SendData = async () => {
        ...
        try {
            ...
            ({
                method:"post",
                url:skinUrl+"rest/registration-affiliate.php",
    ...
    const getCode = async () => {
        ...
        try {
          ...
          const data = await api
            .get('rest/getpromotercodefromchild/:' + affiliate_id + '/')
            .then(response => {
    
              if (response.data.status == "ok") {
    
                setCodicePromotore(response.data.codice);
    
              } else if (response.data.status == "error") {
    
                setCodicePromotore("nocode")
              } else {
    ...
    return (
        <>
            ...
            {!loader ? 
                            ...
                            <div className={SKIN["default_promo"] ? "col-sm-8" : "col-sm-12"}>
                                ...
                                <div className="row margin-regulation2">
                                    <div className="col-sm-12 pd-l-2 pd-r-2">
                                        ...
                                        <input type="text" className="form-control margin-bottom-5" id="promoter_code" name="promoter_code" placeholder={"*" + t('codicepromotore')} value={codicePromotore} onChange={handleChange} readOnly/>
                                    </div>
                                </div>
                ...
                <div>Stai tentando di iscriverti attraverso un codice affiliato insesistente</div>
...
export default RegistrationModalAffiliate;